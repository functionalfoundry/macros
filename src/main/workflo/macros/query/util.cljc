...
(defn backref-attr?
  "Returns true if an attribute is a backref attribute, that is,
   if it has a namespace that starts with `_`."
  [attr]
  (let [ns (namespace attr)
        nm (name attr)]
    (and ns (string/starts-with? ns "_"))))

(defn transform-backref-attr
  "Transforms a backref attribute by moving the `_` from the namespace
   to the name part, e.g. `:_foo/bar` becomes `:bar/_foo`. The former
   is generated by our query language, the latter is used in Om Next
   queries and is consistent with how Datomic represents reverse
   references in e.g. the Datomic Console."
  [attr]
  {:pre [(backref-attr? attr)]}
  (let [ns (namespace attr)
        nm (name attr)]
    (keyword (subs ns 1) (str "_" nm))))
